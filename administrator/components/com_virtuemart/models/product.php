 ...
 * @version $Id: product.php 8845 2015-05-04 20:42:18Z Milbo $
 */
...
// 			$joinLang = false;
	...
	*/
	...
	function getOrderByList ($virtuemart_category_id = FALSE) {
		...
		if (count ($fields) > 1) {
			foreach ($fields as $field) {
				if ($field != $orderby) {
					...
					$orderByLink .= '<option title="' . $text . '" value="' . $link . '">' . $text . '</option>';
				}
			}
		}
		...
		$orderByList = '<div class="select-style"><div class="title">' . vmText::_ ('COM_VIRTUEMART_ORDERBY') . '</div><select onchange="this.options[this.selectedIndex].value && (window.location = this.options[this.selectedIndex].value);"><option selected title="' . $orderDirTxt . '" value="' . $link . '">' . vmText::_ ('COM_VIRTUEMART_SEARCH_ORDER_' . $orderby) .'</option>';
...
die;
	if(VmConfig::get('lstockmail',TRUE)){
		...
		foreach($superUsers as $user_id) {
			...
			//echo $user->email;
			shopFunctionsF::renderMail ('productdetails', $user->email, $vars, 'productdetails', TRUE) ;
		}
