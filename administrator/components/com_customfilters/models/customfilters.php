...
class CustomfiltersModelCustomfilters extends JModelList{
	...
	protected function getListQuery($use_filters=true)
	{
		...
		$query->select('vmc.'.$vmCompatibility->getColumnName('custom_field_desc','#__virtuemart_customs').' AS custom_descr');
		...
		if($use_filters){
			...
			if (!empty($search)) {
				if (stripos($search, 'id:') === 0) {
					...
					$where[]='(vmc.'.$vmCompatibility->getColumnName('custom_title','virtuemart_customs').' LIKE '.$search.' || vmc.'.$vmCompatibility->getColumnName('custom_field_desc','virtuemart_customs').' LIKE '.$search.')';
				}
			}
