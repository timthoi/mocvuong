 ...
 *
 */
class plgVmShipmentWeight_countriesstates extends vmPSPlugin {
	/**
	...
	protected function checkConditions ($cart, $method, $cart_prices) {
		...
		$countriesstates=$countries=$states=array();
		if (!empty($method->countriesstates)) {
			if (!is_array ($method->countriesstates)) {
				$countriesstates[0] = $method->countriesstates;
			} else {
				$countriesstates = $method->countriesstates;
			}
			//[0] => 43_state_730     [1] => 43_state_731     [2] => 43_state_732 
			foreach($countriesstates as $sel){
				if(JString::strpos($sel,'state')!==false){
					list($c,$s)=explode("_state_",$sel);
					$states[$c][]=$s;
				}elseif(JString::strpos($sel,'all')!==false){
					list($c,$s)=explode("_",$sel);
					$states[$c][]='all';
				}else{
					if((int)$sel>0) $countries[]=$sel;
				}
			}
		}
	
		if (!count($address) || empty($address['zip']) ||  empty($address['virtuemart_country_id']) ||  empty($address['virtuemart_state_id'])){
			if(empty($address['zip'])) $address['zip']=$app->input->getInt('zip_field',0);
			if(empty($address['virtuemart_country_id'])) $address['virtuemart_country_id']=$app->input->getInt('virtuemart_country_id',0);
			if(empty($address['virtuemart_state_id'])) $address['virtuemart_state_id']=$app->input->getInt('virtuemart_country_id',0);
		}
		...
		$cCntry=(int)$address['virtuemart_country_id'];
		$cState=(int)$address['virtuemart_state_id'];

		$countrystate_cond=false;
		if( (isset($states[$cCntry]) && count($states[$cCntry])>0 && in_array($cState, $states[$cCntry])) || //States options
			(isset($states[$cCntry]) && count($states[$cCntry])>0 && in_array('all', $states[$cCntry])) || //ALL states selected options
			(count($countries)>0 && in_array($cCntry, $countries) ) //Country without states
		 ) $countrystate_cond=true;
	
		$allconditions = (int) $weight_cond + (int)$zip_cond + (int)$nbproducts_cond + (int)$orderamount_cond + (int)$countrystate_cond;
		if($allconditions === 5){
			$result[$hash] = true;
			return TRUE;
		} else {
			$result[$hash] = false;
			//vmdebug('checkConditions '.$method->shipment_name.' does not fit ',(int)$weight_cond,(int)$zip_cond,(int)$nbproducts_cond,(int)$orderamount_cond,(int)$countrystate_cond);
			return FALSE;
		}
