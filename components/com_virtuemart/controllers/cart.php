...
class VirtueMartControllerCart extends JControllerLegacy {
	...
	public function updateShipTo_ajax(){
		$cart = VirtueMartCart::getCart();
		$cart->_fromCart = true;
		...
		$cart->setCartIntoSession(true,true);
		 ...
		 //echo json_encode($cart->getCartDataToStore());
		 exit();
	}
	...
	public function updatecart($html=true,$force = null){
		...
		$virtuemart_user_id = $jinput->get('virtuemart_user_id', '', 'INT');
		$virtuemart_userinfo_id = $jinput->get('virtuemart_userinfo_id', '', 'INT');
		
		if ( $virtuemart_user_id!="" || $virtuemart_userinfo_id!="" ):
			$first_name = $jinput->get('first_name', '', 'STR');
			$last_name = $jinput->get('last_name', '', 'STR');
			$address_1 = $jinput->get('address_1', '', 'STR');
			$zip = $jinput->get('zip', '', 'INT');
			$city = $jinput->get('city', '', 'STR');
			$virtuemart_country_id = $jinput->get('virtuemart_country_id', '', 'INT');
			$phone_1 = $jinput->get('phone_1', '', 'INT');
		
			$dbo = Jfactory::getDbo();
			if(empty($virtuemart_userinfo_id)) {
				$query = "INSERT INTO #__virtuemart_userinfos(`virtuemart_user_id`,`address_type`, `first_name`,`last_name`,`address_1`, `zip`, `city`, `virtuemart_country_id`, `phone_1`) VALUES('{$virtuemart_user_id}', 'BT', '{$first_name}', '{$last_name}', '{$address_1}', '{$zip}', '{$city}', '{$virtuemart_country_id}', '{$phone_1}')";
				$dbo->setQuery($query);
				$dbo->execute();
			} else {
	...
	public function setcoupon() {
		...
		if ($cart) {
			...
			if (!empty($coupon_code)) {
				...
				if ( $return == "payment")	$app->redirect(JRoute::_('index.php?option=com_virtuemart&view=cart&task=editpayment', FALSE));
				else
					$app->redirect(JRoute::_('index.php?option=com_virtuemart&view=user&task=editaddresscart&addrtype=BT', FALSE),$msg);
