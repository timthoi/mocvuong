...
// if joomla link joomlalink/vendorname/layout
	...
	if ( $helper->compareKey($view,'orders') || $helper->activeMenu->view == 'orders') {
		...
		if ( !empty($segments) ) {
			...
			elseif (  $helper->compareKey($segments[0] ,'list_address_user') ) {
				$vars['layout'] = 'list_address_user' ;      //uncomment and lets test
			}
			elseif (  $helper->compareKey($segments[0] ,'list_order_user') ) {
				$vars['layout'] = 'list_order_user' ;      //uncomment and lets test
			}
			elseif (  $helper->compareKey($segments[0] ,'edit_address_user') ) {
				$vars['layout'] = 'edit_address_user' ;      //uncomment and lets test
			}
