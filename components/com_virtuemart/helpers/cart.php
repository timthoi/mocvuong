 ...
 * @version $Id: cart.php 8838 2015-04-23 21:39:46Z Milbo $
 */
...
class VirtueMartCart {
	...
	*/
	...
	/* 	else{
			$app = JFactory::getApplication();
			$app->redirect(JRoute::_('index.php?option=com_virtuemart&view=cart&task=editpayment',$this->useXHTML,$this->useSSL));		
		} */
	...
	public function checkoutData($redirect = true) {
		...
		if( !empty($this->cartPrices) && $this->cartPrices['salesPrice']>0.0){
			if (empty($this->virtuemart_paymentmethod_id)) {
				//dont have payment redirect to payement page
	...
	*/
	...
	function prepareCartData($force=true){
		...
		/* var_dump($this->virtuemart_shipmentmethod_id);
		die; */
	...
	function prepareAjaxData($checkAutomaticSelected=true){
		...
		foreach ($this->products as $i=>$product){
			...
			$data->products[$i]['product_link'] = $url;
