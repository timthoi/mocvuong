			...
			foreach($customFilters as $cf){
				...
				if($cf->disp_type!=5 && $cf->disp_type!=6 && $cf->disp_type!=8){//if not range
					...
					//plugins
					{
						...
						}else{//if not string is number and already filtered in the input
			...
			foreach ($tokens as $index=>$token){
				...
				foreach ($searchfields as $searchfield){
					switch ($searchfield){
						 ...
						 * The sku should not use the phrase in reversed order.
	...
	public function getOrderByList($virtuemart_category_id=false) 
	{
		...
		if (count($fields)>0) {
			foreach ($fields as $field) {
				...
				$text = JText::_('COM_VIRTUEMART_'.strtoupper($fieldWithoutPrefix).'_ASC');
				$link = $this->getOrderURI($field,$selected,'ASC');
				
				$filter_order_orther  = "DESC";
				$text1 = JText::_('COM_VIRTUEMART_'.strtoupper($fieldWithoutPrefix).'_DESC');
				$link1 = $this->getOrderURI($field,true,'DESC');
				...
				if(!$selected){
					$orderByLinks .='<option title="'.$text.'" value="'.$link.'" >'.$text.'</option><option title="'.$text1.'" value="'.$link1.'" >'.$text1.'</option>';
				}
				else{
					if($order == "ASC"){
						$first_optLink='<option selected class="activeOrder" title="'.$text.'" value="'.$link.'" >'.$text.'</option> <option title="'.$text1.'" value="'.$link1.'" >'.$text1.'</option>';
					}
					else{
						$first_optLink='<option title="'.$text.'" value="'.$link.'" >'.$text.'</option> <option selected class="activeOrder" title="'.$text1.'" value="'.$link1.'" >'.$text1.'</option>';
					}
		...
		$orderByHtml='<select onchange="this.options[this.selectedIndex].value && (window.location = this.options[this.selectedIndex].value);" class="orderlist">'.$first_optLink.$orderByLinks.'</select>';
