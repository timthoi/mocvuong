	...
	if ($result > 0 ){
			...
			else
			{
				...
				else 
				{
					...
					$addtofavorites .= 'value="" name="addtofavorites" title="'.JText::_('VM_ALL_FAVORITE_PRODUCTS').'" >';
	...
	if ($result == 0){
		if($product_id == $favorite_id && $mode == "fav_add") {
			...
			if ($guest_enabled || !$user->guest){
					$addtofavorites .= '
					<form style="display: inline-block; text-align: center; margin:0px" method="post" action="'.$url_fav.'" name="addtofavorites" id="addtofavorites_'.$product_id.'">';
					...
					$addtofavorites .= 'value="" name="addtofavorites" title="'.JText::_('VM_ADD_TO_FAVORITES').'" >';
