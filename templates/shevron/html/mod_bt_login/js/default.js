...
BTLJ(document).ready(function() {
	...
	BTLJ(document).click(function(event){
		if(requireRemove && event.which == 1) btTimeOut = setTimeout('BTLJ("#btl-content > div").hide();BTLJ(".btl-panel span").removeClass("active");',10);
		requireRemove =true;
	})
...
function registerAjax(){
	...
	var datasubmit = BTLJ('form[name=btl-formregistration]').serialize();
			  ...
			  if(html.indexOf('$error$')!= -1){
				  ...
				  if(btlOpt.RECAPTCHA =="1"){
				   ...
				   var token = BTLJ('.btl-buttonsubmit input:last').attr("name");
					...
					var datasubmit= "bttask=login&username="+encodeURIComponent(BTLJ(".btl-formregistration #btl-input-email1").val())
					+"&passwd=" + encodeURIComponent(BTLJ(".btl-formregistration #btl-input-password1").val())
					+ "&"+token+"="+value_token;
					...
					success: function (html, textstatus, xhrReq){
						if(html == "1" || html == 1){
							window.location.href= 'index.php?option=com_virtuemart&view=user&layout=edit';
						}else{
...
function loginAjax(){
	...
	BTLJ.ajax({
	   ...
	   success: function (html, textstatus, xhrReq){
		  if(html == "1" || html == 1){
			   BTLJ.get('index.php?option=com_ajax&module=breadcrumbs&method=redirectLoggedUser&format=json&current_url='+btlOpt.BT_RETURN,
			   function(res) {
				   var response = BTLJ.parseJSON(res);
