...
class ModCfilteringOptions
{
	 ...
	 * @author	Sakis Terz
	 */
	public function buildCategoriesQuery($query,$part=false)
	{
		$where=array();
		$where_str='';
		$innerJoin=array();
	 ...
	 *
	 * @return	Array	the ranges
	 * @since	2.2.2
	 */
	public function getPriceRanges()
	{
		//try using external cache - if exists
		$store_id='price_range_init';
		...
		if(empty($results)){
			...
			if(!empty($ranges)){
				...
				if(!empty($ranges->min_value))$ranges->min_value=floor($ranges->min_value);
				if(!empty($ranges->max_value))$ranges->max_value=round($ranges->max_value);
	...
	public function getRelativePriceRanges()
	{
		...
		if(empty($this->selected_flt) || (!empty($this->selected_flt['virtuemart_category_id']) && count($this->selected_flt)==1) ||  (!empty($this->selected_flt['virtuemart_category_id']) && !empty($this->selected_flt['price']) && count($this->selected_flt)==1))$use_external_cache=true;
		if((!empty($this->selected_flt['virtuemart_manufacturer_id']) && count($this->selected_flt)==1) ||  (!empty($this->selected_flt['virtuemart_manufacturer_id']) && !empty($this->selected_flt['price']) && count($this->selected_flt)==1))$use_external_cache=true;

		/*Get ranges from externalcache*/
		if($use_external_cache){
			$store_id=md5('price_range::'.json_encode($this->selected_flt));
			$cache=JFactory::getCache('mod_cf_filtering.ranges','output');
			$lt=(int)$this->componentparams->get('cache_time',180);
			$cache->setCaching( 1 );
			$cache->setLifeTime($lt);
		...
		if(empty($ranges)){
			...
			if(is_array($ranges))$ranges=reset($ranges);
			$ranges=$this->caclFinalPrice($ranges);
			if(!empty($ranges->min_value))$ranges->min_value=floor($ranges->min_value);
			if(!empty($ranges->max_value))$ranges->max_value=round($ranges->max_value);
			if($use_external_cache)$cache->store($ranges,$store_id);
		}
	...
	public function caclFinalPrice($ranges)
	{
		...
		if((int)$vendor_currency['vendor_currency'] !=$shop_currency_id){
			...
			if(!empty($ranges->min_value))$ranges->min_value=$vmCurrencyHelper->convertCurrencyTo($ranges->min_value,$shop_currency_id,$shop=false);
			if(!empty($ranges->max_value))$ranges->max_value=$vmCurrencyHelper->convertCurrencyTo($ranges->max_value,$shop_currency_id,$shop=false);
		}

		$calc_rules=cftools::getCalcRules();
		if(empty($calc_rules))return $ranges;
		...
		if(!empty($ranges->max_value)){
			...
			if($min_value_tmp==$ranges->max_value)$ranges->max_value=$ranges->min_value;
			else $ranges->max_value=$this->addCalcRulesByCalcType($ranges->max_value,$calc_rules);
		}
	 ...
	 * @since 2.2.2
	 */
	public function addCalcRulesByCalcType($price, $calc_group)
	{
	...
	public function addCalcRule($price, $calc )
	{
		...
		if($value!=0){
			...
			if (strlen($mathop) == 2) {
				...
				if((int)$vendor_currency['vendor_currency']!=$shop_currency_id){
					if(!empty($ranges->min_value))$calculated=$vmCurrencyHelper->convertCurrencyTo($calculated,$shop_currency_id,$shop=false);
				}
			}
	 ...
	 * @since	2.2.2
	 */
	public function buildPriceRangeQuery($query,$part=false)
	{
	 ...
	 * @since	1.5.0
	 */
	public function buildCustomFltQuery($query,$customfilter,$part=false)
	{
		$vmCompatibility=VmCompatibility::getInstance();
	...
	public static function getDisplayControl($flt_sfx, $selected_flt,$params)
	{
		...
		if($is_published){
			...
			if($disp){
				...
				else if($disp_with_fltrs=='keyword'){
					if(isset($selected_flt['q']))$disp=true;
					else $disp=false;
				}
				...
				else if($disp_with_fltrs=='keyword_or_vm_cat_or_customfilter'){
					if(isset($selected_flt['q']) || isset($selected_flt['virtuemart_category_id'])  || isset($selected_flt['virtuemart_manufacturer_id']))$disp=true;
					else $disp=false;
				}

				//display  if keyword ,category , a manuf or a price is selected
				else if($disp_with_fltrs=='keyword_or_vm_cat_or_vm_manuf'){
					if(isset($selected_flt['q']) || isset($selected_flt['virtuemart_category_id'])  || isset($selected_flt['virtuemart_manufacturer_id']))$disp=true;
					else $disp=false;
				}

				//display  if keyword ,category , a manuf or a price is selected
				else if($disp_with_fltrs=='keyword_or_vm_cat_or_vm_manuf_or_price'){
					if(isset($selected_flt['q']) || isset($selected_flt['virtuemart_category_id'])  || isset($selected_flt['virtuemart_manufacturer_id']) || isset($selected_flt['price']))$disp=true;
					else $disp=false;
				}

				//display  if keyword or category and manuf is selected
				else if($disp_with_fltrs=='keyword_or_vm_cat_and_vm_manuf'){
					if((isset($selected_flt['q'])  || isset($selected_flt['virtuemart_category_id'])) && isset($selected_flt['virtuemart_manufacturer_id']))$disp=true;
					else $disp=false;
				}
				else if($disp_with_fltrs=='keyword_or_vm_cat_or_customfilter'){
					if(isset($selected_flt['q']) || isset($selected_flt['virtuemart_category_id']) || strpos($selected_filters_keys_str, 'custom_f_')!==false)$disp=true;
					else $disp=false;
				}
			}
			return $disp;
