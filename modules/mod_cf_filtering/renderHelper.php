...
class ModCfilteringRender
{
	...
	public function renderFilters()
	{
		...
		foreach($this->filters as $key=>$flt){
			...
			if(count($options_ar)>0){
				foreach($disp_typeArray as $disp_type){
					...
					//--Range dates--//
						...
						$format = $dateFormat;
						$format_php=$dateFormat_php;
						$today=date($format_php);
						//initialization of some vars

						$maxlength=10;
						$size=9;

						$val_from=!empty($this->selected_flt[$key][0])?$this->selected_flt[$key][0]:'';
						$val_to=!empty($this->selected_flt[$key][1])?$this->selected_flt[$key][1]:'';
						$script='';
						...
						$innerHTML.='<label for="'.$display_key.'_from"></label>';
						$innerHTML.=
						JHTML::calendar(
						$val_from,
						$key.'[0]',
						...
						$innerHTML.='<label for="'.$display_key.'_to">'.JText::_('MOD_CF_RANGE_TO').'</label>';
						$innerHTML.=
						JHTML::calendar(
						$val_to,
						$key.'[1]',
						...
						if($this->results_trigger!='btn' && ($val_from || $val_to)){//generate the clear link
					...
					//--Range Slider--//
						...
						if($innerHTML){
							...
							$html.='<div class="bg-range"></div><div id="'.$display_key.'_slider_bkg_img" class="slider_bkg_img"></div>';
							$html.=$innerHTML;
					...
					//-- Inputs--//
						...
						foreach ($options_ar as $index=>$option){
							...
							if(isset($this->selected_flt[$key]) && is_array($this->selected_flt[$key]))$value=!empty($this->selected_flt[$key][$index])?$this->selected_flt[$key][$index]:'';
							else $value=!empty($this->selected_flt[$key])?$this->selected_flt[$key]:'0';

							$size=!empty($option->size)?$option->size:7;
							$maxlength=!empty($option->maxlength)?$option->maxlength:7;
							$label=!empty($option->label)?$option->label:'Price:';
							...
							$innerHTML.= $symbol_end.'<input name="'.$option->name.'" value="'.$value.'" type="search" placeholder="'.$placeholder.'" size="'.$size.'" maxlength="'.$maxlength.'" id="'.$display_key.'_'.$index.'" '.$script.'>';
						...
						if($innerHTML){
							...
							$lele = '';
							$lele.='<div class="cf_wrapper_input_text '.$input_class.'" id="cf_wrapper_input_text_'.$display_key.'">';
							$lele.='<div class="form-horizontal">';
							$lele.=$innerHTML;
							...
							$lele.=$innerHTML_clear;
							//assigne event to the button
							$this->scriptProcesses[]="customFilters.assignEvents(".$this->module->id.");";

							$lele.='</div>';
							$lele.='<div class="cf_message" id="'.$display_key.'_message"></div>';
							$lele.='<input type="hidden" value="'.$clear_targ.'" id="'.$display_key.'_url'.'"/>';
							$lele.='</div>';
						}
						else {
							$lele = '';
		 ...
		 $filters_html_array[$key.'_'.$this->module->id]=$html.$lele;
		}
	...
	public function setSliderScripts($filter_key,$inputs=false,$slider_min_value=0,$slider_max_value=100)
	{
		...
		else if(!empty($setMax) && empty($setMin)){
		...
		}else if(!empty($setMin) && !empty($setMax)){
			$this->scriptProcesses[]=" {$display_key}_sliderObj.setMin($setMin);";
			$this->scriptProcesses[]=" {$display_key}_sliderObj.setMax($setMax);";
		}
	}
