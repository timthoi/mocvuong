...
class ModCfFilteringHelper{
	...
	public function getFilters($params, $module){
		...
		$filters_order=array('q','virtuemart_category_id','virtuemart_manufacturer_id','price','custom_f');
			...
			switch($filter_key){
				...
				case 'q':
					if($this->optHelper->getDisplayControl('keyword_flt',$this->selected_flt,$params)){
						 ...
						 */
						if(!$params->get('keyword_flt_display_header',0))$keyword_flt['options'][0]['placeholder']=JText::_('MOD_CF_KEYWORD_SEARCH_PLACEHOLDER');
						$keyword_flt['options'][0]['name']='q';
						$keyword_flt['options'][0]['size']=30;
						$keyword_flt['options'][0]['maxlength']=40;
							
						if($params->get('keyword_flt_display_header',0))$this->filters_headers_array[$display_key]=JText::_('MOD_CF_KEYWORD');
						$this->expanded[$display_key]=$params->get('keyword_flt_expanded','1');
						$this->filters_opt[$filter_key]=$keyword_flt;
						if($profilerParam)$profiler->mark('keyword');
					}
					break;
				...
				case 'virtuemart_category_id':
					if($this->optHelper->getDisplayControl('category_flt',$this->selected_flt,$params)){
						...
						if($vm_cat_disp_type!=1)$this->smartSearch[$key]=$params->get('category_flt_smart_search','0');
				...
				case 'price':
					if($this->optHelper->getDisplayControl('price_flt',$this->selected_flt,$params)){
						...
						if($display_price_inputs || $display_price_slider){
							...
							if($display_price_inputs)$this->scriptProcesses[]="customFilters.addEventsRangeInputs('$filter_key',$module->id);";
							...
							if(!empty($this->currency_info))$this->scriptVars['currency_decimal_symbol']=$this->currency_info->currency_decimal_symbol;
							...
							if($display_price_inputs && !$display_price_slider)$price_flt_disp_type=5;//range input
							else if($display_price_inputs && $display_price_slider){
								$price_flt_disp_type='5,6';
							}
							else $price_flt_disp_type=6;//slider
							...
							if($display_price_slider && $params->get('price_flt_dynamic_ranges',0)){
								if(count($selected_flt)==0 || (count($selected_flt)==1 && !empty($selected_flt['price'])));//$ranges=$this->optHelper->getOptions('price'); //when there is no selection
								else $ranges=$this->optHelper->getRelativePriceRanges();
								if(!empty($ranges->min_value))$min_range=$ranges->min_value;
								if(!empty($ranges->max_value))$max_range=$ranges->max_value;
							...
							$this->filters_opt[$filter_key]=$price_flt;
						}
						if($profilerParam)$profiler->mark('price_flt');
					}
					break;
					...
					if($this->optHelper->getDisplayControl('custom_flt',$this->selected_flt,$params)){
						...
						foreach($custom_flt as $cf){
							...
							if($cf->disp_type!=1){
								...
								if($maxHeight)$this->stylesDeclaration.=" #cf_list_$display_key { max-height:$maxHeight; overflow:auto; height:auto;}";
								if($cf->disp_type!=9 && $cf->disp_type!=10)	$this->smartSearch[$key]=$cfparams->get('smart_search','0'); //no color buttons
							}

							$this->expanded[$display_key]=$cfparams->get('expanded','1');
							$this->disp_types[$var_name]=$cf->disp_type;
							if($cf->disp_type!=5 && $cf->disp_type!=6 && $cf->disp_type!=8)$this->setFilter($name=$var_name,$header=JText::_($cf->custom_title),$cf,$encoded_varID=true);
							//range
							else {
								//check if it should be displayed based on the filter's settings
								if(!$this->optHelper->displayCustomFilter($cf))continue;
								if($cf->disp_type==6){
									//the renderer creates the inputs and the sliders
								}
								if($cf->disp_type==5)$this->scriptProcesses[]="customFilters.addEventsRangeInputs('$key',$module->id);";
								if($cf->disp_type==8){
							...
							if($profilerParam)$profiler->mark($var_name);
						}
					}
					break;
			}//switch
	...
	public function setFilter($var_name,$header, $customfilter=null,$encoded_var=false){
		...
		if(is_array($options_ar) && count($options_ar)>0){
			...
			if($disp_type==1 || $disp_type==2 || $disp_type==4 || $disp_type==7 || (($disp_type==3 || $disp_type==10 || $disp_type==11 || $disp_type==12) && $disp_clear_tool==1 && isset($selected_flt[$var_name]))){
